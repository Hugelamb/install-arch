...
# User chooses the graphics driver for their system (function).
graphics_selector () {
    info_print "List of graphics card types:"
    info_print "1) Nvidia Open-Source Driver"
    info_print "2) Nvidia NV190 series: nvidia-open-dkms"
    info_print "3) Other generic card: vesa"
    input_print "Please select the number of the corresponding graphics provider (e.g. 1): " 
    read -r gpu_choice
    case $gpu_choice in
        1 ) gpu="nvidia-open-dkms"
            return 0;;
        2 ) gpu="nvidia-open-dkms"
            return 0;;
        3 ) gpu="vesa"
            return 0;;
        * ) error_print "You did not enter a valid selection, please try again."
...
# User chooses the graphics
until graphics_selector; do : ; done
...
# Wish to create an EFI partition of 1GiB 
# Root Partition of size 50GiB
# Home Partition using rest of drive
